...
function renderHistory(items){if(!Array.isArray(items))return;box.innerHTML='';renderedIds.clear();if(!items.length){box.innerHTML='<div class="chat-empty"><b>No messages yet</b><span>Start the conversation.</span></div>';return}items.forEach(render);box.scrollTop=box.scrollHeight}
